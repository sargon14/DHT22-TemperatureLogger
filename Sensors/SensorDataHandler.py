...
class SensorDataHandler():
    ...
    def readAndStoreSensorReadings(self):
        ...
        try:
            ...
            self.logger.info(
                'Successfully read: %s sensors. Failed to read: %s sensor(s)',
                len(self.readingsFromSensors), len(self.failedSensors))
        except Exception as e:
            self.logger.error("Sensor reading raised exception", exc_info=True)
            raise

        # Send warning to indicate that some of the configured sensors are not
        # providing correct information
        try:
            # Check if all sensors failed to get readings.
            ...
            self.logger.error(
                "Failed to check if all sensors failed to get readings",
                exc_info=True)
            raise

        try:
            # Check how many sensors failed. If there is more than 0,
            # log and try to send warning mail
            ...
            self.logger.error(
                "Failed to check how many sensors failed to provide readings",
                exc_info=True)
            raise
        ...
        try:
            ...
            self.logger.error("Failed to persist read data to database",
                              exc_info=True)
            raise

        # Compare measured value with prev. measured value and set threshold.
        # E.g. if config has threshold set to 5. Last measurement was 20 and
        # now it is 30. Set threshold has been exceeded by 5 (30-20-5)
        ...
        try:
            ...
            self.logger.error("Failed to compare read value with set limits",
                              exc_info=True)
            raise
        ...
        for key, value in self.readingsFromSensors.iteritems():
            ...
            if (value['lastMeasuredTemperature'] != "") or \
                ...
                try:
                    ...
                    # First check if temperature has changed more than it is
                    # allowed in configuration threshold
                    sendWarning, messageToSend = \
                        self.compareMeasurements.checkTemperatureChange()
                ...
                if sendWarning is True and self.mailSender is not None:
                    try:
                        self.logger.info('Send Sensor warning mail')
                        # Send sensor warning email
                        self.mailSender.sendSensorWarningEmail(
    ...
    def _compareReadValuesWithSetLimits(self):
        ...
        for key, value in self.readingsFromSensors.iteritems():
            ...
            # If betweelimit flag was set to False during temperature or
            # humidity check then start sending warning email
            if betweenLimits is False:
                # Check if lastMailSent is empty
                if not value['lastMailSent']:
                    self.logger.warning('No value in lastMailSent. Send mail.')
                    # If it is empty, no need to compare time to anything,
                    # just send mail
                    mailSendingtimeOutPassed = True
                else:
                    # Compare time delta between last mail sent and current
                    # time. If delta exceeds mailSendingTimeoutInFullHours
                    # value, return TRUE, otherwise FALSE
                    self.logger.info(
                        'Delta check current time and last mail sent time.')
                    try:
                        mailSendingtimeOutPassed = \
                            self.mailSender.checkMailTimeout(
                                value['lastMailSent'])
                    except Exception as e:
                        self.logger.error(
                            'Delta check failed\n', exc_info=True)
                ...
                # If time for last mail sending was found and it passed delta
                # check and mailsender is available, send mail
