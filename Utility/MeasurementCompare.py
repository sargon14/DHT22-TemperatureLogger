...
class MeasurementCompare():
    ...
    def checkTemperatureLimits(self):
        ...
        # If measured temperature is less than low limit -> set betweenlimits
        ...
        if float(self.measuredTemperature) < float(self.temperatureLowLimit):
            ...
            # warning needs to be sent
            self.betweenLimits = False
            self.logger.warning(
                "Measured temperature is lower than set limit")

        # If measured temperature is greater than high limit ->
        # set betweenlimits to False and warning gets sent
            ...
            self.betweenLimits = False
            self.logger.warning(
                "Measured temperature is higher than set limit")
    ...
    def checkHumidityLimits(self):
        ...
        # If measured humidity is less than low limit ->
        # set betweenlimits to False and warning gets sent
        ...
        # If measured humidity is greater than high limit ->
        # set betweenlimits to False and warning gets sent
    ...
    def checkTemperatureChange(self):
        ...
        # check if previously measured humidity is castable to float,
        # if not then measurement comparisons would fail
        if self._isFloat(self.previousTemperature):
            ...
            # measured temperature + allowed threshold
            if self.measuredTemperature > float(self.previousTemperature) + \
                    self.temperatureThreshold:
                # Set sendWarning flag to true to indicate that
                # warning need to be sent
                ...
                self.warningMsg = \
                    "WARNING: Temperature increased more than {0} {1} on sensor {2}\nMeasured temperature: {3} {1}\nPrevious temperature: {4} {1}".format(
                        ...
                        self.previousTemperature)
                self.logger.warning(
                    "Temperature has increased more than set threshold")

            # Check if measured temperature is less than previously
            # measured temperature - allowed threshold
            if self.measuredTemperature < float(self.previousTemperature) - \
                    self.temperatureThreshold:
                # Set sendWarning flag to true to indicate that
                # warning need to be sended
                self.sendWarning = True
                # Compose warning message to be sended out
                self.warningMsg = \
                    "WARNING: Temperature decreased more than {0} {1} on sensor {2}\nMeasured temperature: {3} {1}\nPrevious temperature: {4} {1}".format(
                        ...
                        self.previousTemperature)
                self.logger.warning(
                    "Temperature has decreased more than set threshold")
        else:
            self.logger.warning(
                "...Previous temperature not available, nothing to compare to")
    ...
    def checkHumidityChange(self):
        ...
        if self._isFloat(self.previousHumidity):
            ...
            # measured temperature + allowed threshold
            if self.measuredHumidity > float(self.previousHumidity) + \
                    self.humidityThreshold:
                # Set sendWarning flag to true to indicate that
                # warning need to be sended
                self.sendWarning = True
                ...
                self.logger.warning(
                    "Humidity has increased more than set threshold")

            # Check if measured humidity is less than previously
            # measured temperature - allowed threshold
            if self.measuredHumidity < float(self.previousHumidity) - \
                    self.humidityThreshold:
                # Set sendWarning flag to true to indicate that
