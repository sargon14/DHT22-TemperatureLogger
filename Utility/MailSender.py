...
class MailSender():
    ...
    def sendInformationalEmail(self, msgContent, trigger):
        ...
        try:
            ...
            # Persist mail sending time to mailsendlog.
            # Use trigger to indicate event - it is set to triggedsensor
    ...
    def sendSensorWarningEmail(self, msgContent, sensor, sensorData):
        ...
        try:
            ...
            self.dbController.setLastSensorMailSentTime(
                sensor, sendTime, sensorData['temperature'],
                sensorData['humidity'])
        except:
            self.logger.error(
                'Failed to set mail sent time to database\n', exc_info=True)
    ...
    def checkMailTimeout(self, lastMailSentTime):
        ...
        # check that enough time has passed from when the last warning was sent
        # this check is done so you don't get warning everytime that sensor is
        #  triggered. E.g. sensor is checked every 5 minutes, temperature is
        # lower than trigger -> you get warning every 5 min and mail is flooded
        if float(self.mailSendingTimeout) != 0.0:
            try:
                ...
                self.logger.info(
                    ...
                    float(self.mailSendingTimeout))
                # Compare passed time for timeout value to see if
                # enough full hours have passed
                if passedTime >= float(self.mailSendingTimeout):
                    self.logger.info("Timeout passed, send mail.")
                    return True
                else:
                    self.logger.info(
                        "Still on timeout, mail sending not allowed.")
                    return False
            except:
                raise
        else:
    ...
    def _getDateTimeString(self):
        ...
        try:
            ...
            self.logger.error('Failed to convert mail sending time DateTime \
            Object to string\n', exc_info=True)
            raise
