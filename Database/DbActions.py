...
class DbActions():
    ...
    def sqlBackup(self):
        ...
        if not os.path.exists(backupPathWithDate):
            self.logger.info(
                "Dated backup folder doesn't exist. Creating one...")
            try:
                ...
                if not os.access(dumpPath, os.W_OK):
                    # If write permission isn't set, try adding with subprocess
                    self.logger.warning(
                        "{0} not writeable. Try adding access rights"
                        .format(dumpPath))
                    try:
                        # Start subprocess and execute sudo chmod -R 777 to
                        # folder where dumps are going. This allows writing to
                        # the folder.
                        subprocess.call(
                            ['sudo', 'chmod', '-R', '777', dumpPath])
                    except:
                        self.logger.error(
                            "Failed to grant write permissions to dump path \
                            with subprocess")
                        raise
                    self.logger.info(
            ...
            try:
                # Dump command
                dumpCmd = "mysqldump -u {0} -p{1} {2} > {3}/{4}.sql"\
                    .format(self.user, self.password, self.database,
                            backupPathWithDate, self.database)
